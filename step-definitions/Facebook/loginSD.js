...
Then(/^I verify login button field is enabled$/, async function() {
    expect(await homepage.isLoginBtnEnabled(),'Login button field is NOT enabled').to.be.true;
});
...
When(/^I click login button$/, async function() {
    await homepage.clickLoginButton(), 'Button not clicked';
});

Then(/^I verify error is displayed$/, async function() {
    expect(await loginpage.isLoginErrorDisplayed(), 'Error not displayed').to.be.true;
});
